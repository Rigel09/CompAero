...
class TestPrandtlMeyerClassFuncs:
    ...
    def test_subsonic_nu_from_mach(self):
        assert pm.calcNuFromMach(0.5, self.gamma) == 0

    def test_supersonic_nu_from_mach(self):
        assert pm.calcNuFromMach(1.5, self.gamma) == approx(11.9052, rel=1e-4)

    def test_subsonic_mach_from_nu(self):
        assert pm.calcMachGivenNu(0, self.gamma) == 1.0

    def test_supersonic_mach_from_nu(self):
        assert pm.calcMachGivenNu(11.9052, self.gamma) == approx(1.5, rel=1e-1)
...
class TestPrandtlMeyerClassSubsonic:
    ...
    def test_subsonic_construction_given_mach(self):
        ...
        assert math.isnan(inst.downStreamNu)
        assert math.isnan(inst.downStreamMu)
        assert math.isnan(inst.downStreamMach)
...
class TestPrandtlMeyerClassSupersonic:
    ...
    def test_supersonic_construction_given_deflection_dwnstrm_mach(self):
        ...
        assert inst.downStreamNu == approx(21.90521, rel=1e-4)
        assert inst.downStreamMu == approx(32.9008, rel=1e-4)
        assert inst.downStreamMach == approx(1.84099, rel=1e-4)
