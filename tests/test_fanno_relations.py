...
class TestFannoClassFuncs:
    ...
    def test_subsonic_t_tstar(self):
        assert ffr.calcT_Tstar(0.5, self.gamma) == approx(1.1429, rel=1e-4)

    def test_subsonic_mach_from_t_tstar(self):
        assert ffr.calcMachFrom_T_TSt(1.14285714, self.gamma) == approx(0.5, rel=1e-2)

    def test_subsonic_p_pstar(self):
        assert ffr.calcP_Pstar(0.5, self.gamma) == approx(2.1381, rel=1e-4)

    def test_subsonic_mach_from_p_pstar(self):
        assert ffr.calcMachFrom_P_PSt(2.13808993, self.gamma) == approx(0.5, rel=1e-2)

    def test_subsonic_rho_rhoStar(self):
        assert ffr.calcRho_RhoStar(0.5, self.gamma) == approx(1.871, rel=1e-4)

    def test_subsonic_mach_from_rho_rhoStar(self):
        assert ffr.calcMachFrom_Rho_RhoSt(1.871, 1.4) == approx(0.5, 1e-3)

    def test_subsonic_p0_p0Star(self):
        assert ffr.calcPo_PoStar(0.5, self.gamma) == approx(1.3398, rel=1e-4)

    def test_subsonic_mach_from_p0_p0Star(self):
        assert ffr.calcMachFrom_Po_PoSt(1.33984375, self.gamma, flowType=FS.SUB_SONIC) == approx(0.5, 1e-3)

    def test_subsonic_4FLstarD(self):
        assert ffr.calc4FLSt_D(0.5, self.gamma) == approx(1.0691, rel=1e-4)

    def test_subsonic_mach_from_4FLstarD(self):
        assert ffr.calcMachFrom_4FLSt_D(1.06906031, self.gamma, flowType=FS.SUB_SONIC) == approx(
            0.5, rel=1e-3
        )

    def test_subsonic_u_uStar(self):
        assert ffr.calcU_Ustar(0.5, self.gamma) == approx(0.5345, rel=1e-4)

    def test_subsonic_mach_from_u_uStar(self):
        assert ffr.calcMachFrom_U_USt(0.53452248, self.gamma) == approx(0.5, rel=1e-3)

    # Test the Functions for Supersonic Case
    #######################################################################################
    def test_supersonic_t_tstar(self):
        assert ffr.calcT_Tstar(1.5, self.gamma) == approx(0.82759, rel=1e-4)

    def test_supersonic_mach_from_t_tstar(self):
        assert ffr.calcMachFrom_T_TSt(0.82758620, self.gamma) == approx(1.5, rel=1e-2)

    def test_supersonic_p_pstar(self):
        assert ffr.calcP_Pstar(1.5, self.gamma) == approx(0.6065, rel=1e-4)

    def test_supersonic_mach_from_p_pstar(self):
        assert ffr.calcMachFrom_P_PSt(0.60647843, self.gamma) == approx(1.5, rel=1e-2)

    def test_supersonic_rho_rhoStar(self):
        assert ffr.calcRho_RhoStar(1.5, self.gamma) == approx(0.7328, rel=1e-4)

    def test_supersonic_mach_from_rho_rhoStar(self):
        assert ffr.calcMachFrom_Rho_RhoSt(0.7328, 1.4) == approx(1.5, 1e-3)

    def test_supersonic_p0_p0Star(self):
        assert ffr.calcPo_PoStar(1.5, self.gamma) == approx(1.1762, rel=1e-4)

    def test_supersonic_mach_from_p0_p0Star(self):
        assert ffr.calcMachFrom_Po_PoSt(1.17616705, self.gamma, flowType=FS.SUPER_SONIC) == approx(1.5, 1e-3)

    def test_supersonic_4FLstarD(self):
        assert ffr.calc4FLSt_D(1.5, self.gamma) == approx(0.13605, rel=1e-4)

    def test_supersonic_mach_from_4FLstarD(self):
        assert ffr.calcMachFrom_4FLSt_D(0.13605021, self.gamma, flowType=FS.SUPER_SONIC) == approx(
            1.5, rel=1e-3
        )

    def test_supersonic_u_uStar(self):
        assert ffr.calcU_Ustar(1.5, self.gamma) == approx(1.3646, rel=1e-4)

    def test_supersonic_mach_from_u_uStar(self):
        assert ffr.calcMachFrom_U_USt(1.36457647, self.gamma) == approx(1.5, rel=1e-3)
...
class TestFannoClassSubsonic:
    ...
    def test_fanno_from_mach(self):
        ...
        inst.applyPipeParameters(0.4, 11, 0.005)
    ...
    def test_fanno_choked_flow(self):
        inst = ffr(self.gamma, mach=0.5)
        inst.applyPipeParameters(0.4, 22, 0.005)
        assert inst.gamma == approx(self.gamma, rel=1e-3)
        assert inst.mach == approx(0.5, rel=1e-3)
        assert inst.t_tSt == approx(1.1429, rel=1e-4)
        assert inst.p_pSt == approx(2.1381, rel=1e-4)
...
class TestFannoClassSupersonic:
    ...
    def test_fanno_from_mach(self):
        ...
        inst.applyPipeParameters(0.4, 1.5, 0.005)
        assert inst.gamma == approx(self.gamma, rel=1e-3)
