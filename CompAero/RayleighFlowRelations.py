...
class RayleighFlowRelations():
    def __init__(self, gamma: float, mach: float = nan, t_tSt: float = nan, p_pSt: float = nan, rho_rhoSt: float = nan, \
            po_poSt: float = nan, to_toSt: float = nan, u_uSt: float = nan, flowType: str = "Supersonic") -> None:
        self.gamma       = gamma
        self.mach        = mach
        self.t_tSt       = t_tSt
        self.p_pSt       = p_pSt
        ...
        if self.__checkValue(self.t_tSt):
            self.mach = RayleighFlowRelations.calcMachFrom_T_TSt(self.t_tSt, self.gamma, flowType=self.flowType)
            ...
            self.mach = RayleighFlowRelations.calcMachFrom_Po_PoSt(self.po_poSt, self.gamma)

        elif self.__checkValue(self.to_toSt):
            self.mach = RayleighFlowRelations.calcMachFrom_To_ToSt(self.to_toSt, self.gamma, flowType=self.flowType)
    ...
    def simulateHeatAddition(self, heat: float, to1: float, gasConstantR: float) -> None:
        ...
        self.dwnStrmMach       = RayleighFlowRelations.calcMachFrom_To_ToSt(self.dwnStrm_to_toSt, self.gamma, self.flowType)
    ...
    def __str__(self) -> str:
        ...
        if self.heat > self.chokedHeat:
            choked         = "|" + Fore.BLACK + Back.LIGHTYELLOW_EX + "{:<{width}}{}".format("Heat fro Choked Flow", chokedStr, width=width - len(chokedStr)) + Style.RESET_ALL + "|\n"
        else:
            choked         = "|{:<{width}}{}|\n".format("Heat fro Choked Flow", chokedStr, width=width - len(chokedStr))
        ...
        return "".join([header, sep, gamma, tempR, pressR, poR, rhoR, uStR, toR, flow, \
            sep, pipeSep, choked, heat, gasConst, cp, to1, to2, sep, dwnStrmSep, dwmMach, dwnTmp, dwnPress, dwnRho, dwnPor, dwnVel, dwnTor, \
            sep, jumpSep, temp2, press2, rho2, po2, to2R, u2, finish])
    ...
    @staticmethod
    def calcMachFrom_T_TSt(t_tSt: float, gamma: float, flowType: str = "Supersonic") -> float:
        ...
        if t_tSt == 1.0:
            ...
            return brenth(RayleighFlowRelations.calcT_TSt_FromMach, tolerance, 1 - tolerance, args=(gamma, t_tSt,))
        else:
            raise ValueError(Back.RED + Fore.BLACK + "Flow Type [{}] not supported for Fanno"
    ...
    @staticmethod
    def calcMachFrom_Po_PoSt(po_poSt: float, gamma: float) -> float:
        ''' Calculates mach given po/po* and gamma'''
        return brenth(RayleighFlowRelations.calcPo_PoSt_FromMach, 1e-8, 40, args=(gamma, po_poSt,))
    ...
    @staticmethod
    def calcMachFrom_To_ToSt(t_tSt: float, gamma: float, flowType: str = "Supersonic") -> float:
        ...
        if t_tSt == 1.0:
            ...
            return brenth(RayleighFlowRelations.calcTo_ToSt_FromMach, tolerance, 1 - tolerance, args=(gamma, t_tSt,))
        else:
            raise ValueError(Back.RED + Fore.BLACK + "Flow Type [{}] not supported for Fanno"
                " Flow! Accepted Types: 'Supersonic' & 'Subsonic'".format(flowType) + Back.RESET + Fore.RESET)
