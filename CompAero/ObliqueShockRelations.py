...
class ObliqueShockRelations(NormalShockRelations):
    def __init__(self, gamma: float, useDegrees: bool = True, shockAngle: float = nan, wedgeAngle: float = nan, \
                mn1: float = nan, mn2: float = nan, mach: float = nan, p2_p1: float = nan, rho2_rho1: float = nan, \
                t2_t1: float = nan, po2_po1: float = nan, po2_p1: float = nan, m2: float = nan, shockType: str = "Weak") -> None:
        ...
        if self.useDegrees and self.__checkValue(self.shockAngle): self.shockAngle = radians(self.shockAngle)
        if self.useDegrees and self.__checkValue(self.wedgeAngle): self.wedgeAngle = radians(self.wedgeAngle)

        if not self.__checkValue(self.gamma): return
        ...
        if self.__checkValue(self.mach):
            ...
            self.mach = ObliqueShockRelations.calcMachFromThetaBeta(self.shockAngle, self.wedgeAngle, self.gamma)
            ...
            maxDeflectionAngle = ObliqueShockRelations.calculateMaxFlowDeflectionAngle(maxShockAngle, self.mach, self.gamma)

            if machAngle > self.shockAngle:
                raise RuntimeError("Shock Angle, {}, cannot be less than that of Mach Wave, {}".format(round(self.shockAngle, 4), round(machAngle, 4)))

            if maxDeflectionAngle < self.wedgeAngle:
                raise RuntimeError("SHOCK DETACHED!! Deflection Angles: Given: {}\tMax: {}".format(round(self.wedgeAngle, 4), round(maxDeflectionAngle, 4)))


        elif self.__checkValue(m2) and self.__checkValue(self.wedgeAngle) and self.__checkValue(self.shockAngle):
            self.machNorm2 = ObliqueShockRelations.calcMachNormal2FromMach2(m2, self.shockAngle, self.wedgeAngle)
        ...
        if self.__checkValue(self.mach) and ((self.__checkValue(self.shockAngle) or self.__checkValue(self.wedgeAngle))):
    ...
    def __calculateState(self) -> None:
        if self.__checkValue(self.wedgeAngle):
            if self.shockType == "Weak":
                self.shockAngle = ObliqueShockRelations.calcBetaFromThetaMach_Weak(self.wedgeAngle, self.mach, self.gamma)

            elif self.shockType == "Strong":
                self.shockAngle = ObliqueShockRelations.calcBetaFromThetaMach_Strong(self.wedgeAngle, self.mach, self.gamma)

            else:
                raise ValueError("Incorrect shock type specified -> [{}]. Choices: Strong, Weak".format(self.shockType))
        
        elif self.__checkValue(self.shockAngle):
            self.wedgeAngle = ObliqueShockRelations.calcThetaFromBetaMach(self.shockAngle, self.mach, self.gamma)
    ...
    def __str__(self) -> str:
        ...
        top         = "|{:=^{width}}|\n".format("", width=width)
        header      = "|{:=^{width}}|\n".format("Oblique Shock Relations at Mach: {}".format(mach1Str), width=width)
        ...
        Ftype       = "|{:<{width}}{}|\n".format("Flow Turn Type:", self.shockType, width=width - len(self.shockType))

        sep = "|{:{width}}|".format("", width = width)

        shockSep  = "\n|{:-^{width}}|\n".format("Shock Jump Conditions", width=width)
        pRatio    = "|{:<{width}}{}|\n".format("p2/p1:", pRatioStr, width=width - len(pRatioStr))
        rhoRatio  = "|{:-<{width}}{}|\n".format("\u03C12/\u03C11:", rhoRatioStr, width=width - len(rhoRatioStr))
        ...
        return "".join([top, header, sep, upstreamSep, mach1, mn1, wedge, shock, Ftype, sep, shockSep, \
                            pRatio, rhoRatio, tempRatio, poRatio, poprRatio, sep, downSep, mach2, mn2, finish])
    ...
    def calcMachNormal1(mach: float, beta: float) -> float:
        ...
        if abs(beta - machWave) < 1e-5: return 1.0

        if beta < machWave: return nan
    ...
    def calcBetaFromThetaMach_Weak(theta: float, mach: float, gamma: float) -> float:
        ...
        return brenth(ObliqueShockRelations.calcThetaFromBetaMach, 0.0001, maxShockAngle, args=(mach, gamma, theta))
    
    @staticmethod
    def calcBetaFromThetaMach_Strong(theta: float, mach: float, gamma: float) -> float:
        maxShockAngle = ObliqueShockRelations.calculateMaxShockAngle(mach, gamma)
        return brenth(ObliqueShockRelations.calcThetaFromBetaMach, maxShockAngle, radians(90), args=(mach, gamma, theta))
    ...
    def plotThetaBetaMachChart(self) -> None:
        ...
        maxDeflectionAngle = degrees(ObliqueShockRelations.calculateMaxFlowDeflectionAngle(radians(maxShockAngle), mach, self.gamma))
        ...
        for ii in range(weakFlowDeflectionAngles.shape[0]):
            ...
            weakShockAngles[ii] = degrees(ObliqueShockRelations.calcBetaFromThetaMach_Weak(radians(weakFlowDeflectionAngles[ii]), mach, self.gamma))
            strongShockAngles[ii] = degrees(ObliqueShockRelations.calcBetaFromThetaMach_Strong(radians(weakFlowDeflectionAngles[ii]), mach, self.gamma))
        ...
        ax.annotate("Shock {}\nWedge {}".format(round(maxShockAngle, 2), round(maxDeflectionAngle, 2)), xy=(maxDeflectionAngle, maxShockAngle), xytext=(self.wedgeAngle-0.4*self.wedgeAngle, self.shockAngle+0.1*self.shockAngle), arrowprops=dict(facecolor='black', shrink=0.05))
        ax.annotate("Shock {}\nWedge {}".format(round(self.shockAngle, 2), round(self.wedgeAngle, 2)), xy=(self.wedgeAngle, self.shockAngle), xytext=(self.wedgeAngle-0.2*self.wedgeAngle, self.shockAngle-0.4*self.shockAngle), arrowprops=dict(facecolor='black', shrink=0.05))
