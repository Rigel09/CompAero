...
class FannoFlowRelations:
    def __init__(
        ...
        if self.__checkValue(self.t_tSt):
            ...
            self.mach = FannoFlowRelations.calcMachFrom_Rho_RhoSt(
                self.rho_rhoSt, self.gamma
            )
    ...
    def __str__(self) -> str:
        ...
        # Initial Conditions
        header = "|{:=^{width}}|\n".format(
            "Fanno Relations at Mach: {}".format(machStr), width=width
        )
        gamma = "|{:<{width}}{}|\n".format(
            "\u03B3", gammaStr, width=width - len(gammaStr)
        )
        ...
        rhoR = "|{:<{width}}{}|\n".format(
            "\u03C1/\u03C1*", rhoStr, width=width - len(rhoStr)
        )
        lenR = "|{:-<{width}}{}|\n".format(
            "4fL*/D", f4ldStr, width=width - len(f4ldStr)
        )
        uStR = "|{:<{width}}{}|\n".format("U/U*", UStr, width=width - len(UStr))
        flow = "|{:-<{width}}{}|\n".format(
            "FlowType", self.flowType, width=width - len(self.flowType)
        )
        ...
        pipeLen = "|{:-<{width}}{}|\n".format(
            "Pipe Length", pipeLenStr, width=width - len(pipeLenStr)
        )
        pipeDiam = "|{:<{width}}{}|\n".format(
            "Pipe Diameter", diamStr, width=width - len(diamStr)
        )
        fricCoeff = "|{:-<{width}}{}|\n".format(
            "Friction Coefficient", coeffStr, width=width - len(coeffStr)
        )

        # Down Stream Conditions
        dwnStrmSep = "|{:-^{width}}|\n".format("Down Stream Conditions", width=width)
        dwnmachStr = "|{:<{width}}{}|\n".format(
            "Mach", finalmachStr, width=width - len(finalmachStr)
        )
        dwntempR = "|{:-<{width}}{}|\n".format(
            "T/T*", finaltempStr, width=width - len(finaltempStr)
        )
        dwnpressR = "|{:<{width}}{}|\n".format(
            "P/P*", finalprStr, width=width - len(finalprStr)
        )
        dwnpoR = "|{:-<{width}}{}|\n".format(
            "Po/Po*", finalporStr, width=width - len(finalporStr)
        )
        dwnrhoR = "|{:<{width}}{}|\n".format(
            "\u03C1/\u03C1*", finalrhoStr, width=width - len(finalrhoStr)
        )
        dwnlenR = "|{:-<{width}}{}|\n".format(
            "4fL*/D", finalf4ldStr, width=width - len(finalf4ldStr)
        )
        dwnuStR = "|{:<{width}}{}|\n".format(
            "U/U*", finalUStr, width=width - len(finalUStr)
        )

        # Parameter Ratios
        jumpSep = "|{:-^{width}}|\n".format(
            "Conditions Across Friciton Area", width=width
        )
        jumppRatio = "|{:<{width}}{}|\n".format(
            "p2/p1:", jumpprStr, width=width - len(jumpprStr)
        )
        jumprhoRatio = "|{:-<{width}}{}|\n".format(
            "\u03C12/\u03C11:", jumprhoStr, width=width - len(jumprhoStr)
        )
        jumptempRatio = "|{:<{width}}{}|\n".format(
            "T2/T1:", jumptempStr, width=width - len(jumptempStr)
        )
        jumppoRatio = "|{:-<{width}}{}|\n".format(
            "po2/po1:", jumpporStr, width=width - len(jumpporStr)
        )
        jumpFrRatio = "|{:<{width}}{}|\n".format(
            "(4fL*/D)2 / (4fL*/D)1:", jumpf4ldStr, width=width - len(jumpf4ldStr)
        )
        jumpVelRatio = "|{:<{width}}{}|\n".format(
            "U2/U1:", jumpUStr, width=width - len(jumpUStr)
        )
    ...
    def applyPipeParameters(
        self, diameter: float, length: float, frictionCoeff: float = 0.005
    ) -> None:
    ...
    def __calculateState(self) -> None:
        self.t_tSt = FannoFlowRelations.calcT_TSt_FromMach(self.mach, self.gamma)
        self.p_pSt = FannoFlowRelations.calcP_PSt_FromMach(self.mach, self.gamma)
        self.rho_rhoSt = FannoFlowRelations.calcRho_RhoSt_FromMach(
            self.mach, self.gamma
        )
        self.po_poSt = FannoFlowRelations.calcPo_PoSt_FromMach(self.mach, self.gamma)
        self.f4LSt_D = FannoFlowRelations.calc4FLSt_D_FromMach(self.mach, self.gamma)
        self.u_uSt = FannoFlowRelations.calcU_USt_FromMach(self.mach, self.gamma)
    ...
    def __calculateDownStreamState(self) -> None:
        ...
        self.dwnStrm_f4LSt_D = (
            self.f4LSt_D - 4 * self.frictionCoeff * self.pipeLength / self.pipeDiameter
        )
        ...
        self.dwnStrm_t_tSt = FannoFlowRelations.calcT_TSt_FromMach(
            self.dwnStrmMach, self.gamma
        )
        self.dwnStrm_p_pSt = FannoFlowRelations.calcP_PSt_FromMach(
            self.dwnStrmMach, self.gamma
        )
        self.dwnStrm_rho_rhoSt = FannoFlowRelations.calcRho_RhoSt_FromMach(
            self.dwnStrmMach, self.gamma
        )
        self.dwnStrm_po_poSt = FannoFlowRelations.calcPo_PoSt_FromMach(
            self.dwnStrmMach, self.gamma
        )
        self.dwnStrm_u_uSt = FannoFlowRelations.calcU_USt_FromMach(
            self.dwnStrmMach, self.gamma
        )
        self.dwnStrm_f4LSt_D = FannoFlowRelations.calc4FLSt_D_FromMach(
            self.dwnStrmMach, self.gamma
        )
    ...
    def calcMachFrom_T_TSt(t_tSt: float, gamma: float) -> float:
        ...
        return brenth(
            FannoFlowRelations.calcT_TSt_FromMach, 1e-9, 40, args=(gamma, t_tSt,)
        )

    @staticmethod
    def calcP_PSt_FromMach(mach: float, gamma: float, offset: float = 0.0) -> float:
        """Calculates p/p* given gamma and mach"""
        return sqrt(FannoFlowRelations.calcT_TSt_FromMach(mach, gamma)) / mach - offset

    @staticmethod
    def calcMachFrom_P_PSt(p_pSt: float, gamma: float) -> float:
        """ Calculates mach given p/p* and gamma"""
        return brenth(
            FannoFlowRelations.calcP_PSt_FromMach, 1e-9, 40, args=(gamma, p_pSt,)
        )
    ...
    def calcRho_RhoSt_FromMach(mach: float, gamma: float, offset: float = 0.0) -> float:
        ...
        return (
            sqrt(1 / FannoFlowRelations.calcT_TSt_FromMach(mach, gamma)) / mach - offset
        )
    ...
    def calcMachFrom_Rho_RhoSt(rho_rhoSt: float, gamma: float) -> float:
        ...
        return brenth(
            FannoFlowRelations.calcRho_RhoSt_FromMach,
            1e-9,
            40,
            args=(gamma, rho_rhoSt,),
        )
    ...
    def calcPo_PoSt_FromMach(mach: float, gamma: float, offset: float = 0.0) -> float:
        ...
        gm1 = gamma - 1
        return (
            pow(1 / FannoFlowRelations.calcT_TSt_FromMach(mach, gamma), gp1 / (2 * gm1))
            / mach
            - offset
        )
    ...
    def calcMachFrom_Po_PoSt(
        po_poSt: float, gamma: float, flowType: str = "Supersonic"
    ) -> float:
        ...
        if po_poSt == 1.0:
            ...
            return brenth(
                FannoFlowRelations.calcPo_PoSt_FromMach,
                1 + tolerance,
                40,
                args=(gamma, po_poSt,),
            )
            ...
            raise ValueError(
                ...
                " Flow! Accepted Types: 'Supersonic' & 'Subsonic'".format(flowType)
                + Back.RESET
                + Fore.RESET
            )

    @staticmethod
    def calc4FLSt_D_FromMach(mach: float, gamma: float, offset: float = 0.0) -> float:
        """ Calculates 4fL*/D given mach and gamma"""
        gp1 = gamma + 1
        t_tSt = FannoFlowRelations.calcT_TSt_FromMach(mach, gamma)
        mSqr = pow(mach, 2)
        return (
            (1 - mSqr) / (gamma * mSqr)
            + (gp1 / (2 * gamma)) * log(t_tSt * mSqr)
            - offset
        )

    @staticmethod
    def calcMachFrom_4FLSt_D(
        f4lSt_d: float, gamma: float, flowType: str = "Supersonic"
    ) -> float:
        ...
        if f4lSt_d == 0.0:
            ...
            return brenth(
                FannoFlowRelations.calc4FLSt_D_FromMach,
                1.00001,
                50,
                args=(gamma, f4lSt_d,),
            )
    ...
    def calcMachFrom_U_USt(u_uSt: float, gamma: float) -> float:
        ...
        return brenth(
            FannoFlowRelations.calcU_USt_FromMach, 1e-9, 40, args=(gamma, u_uSt)
        )
