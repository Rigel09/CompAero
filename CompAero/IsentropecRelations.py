...
class IsentropicRelations:
    ...
    def __str__(self) -> str:
        ff = "\nIsentropic Flow State at Mach: {}\n".format(round(self.mach, self.precision))
        ff2 = "---------------------------------------\n"
        first = "p0/p:  {}\n".format(round(self.p0_p, self.precision))
        second = "T0/T:  {}\n".format(round(self.t0_t, self.precision))
        third = "\u03C1_0/\u03C1: {}\n".format(round(self.rho0_rho, self.precision))
        fourth = "A/A*:  {}\n".format(round(self.a_aStar, self.precision))
    ...
    @staticmethod
    def calc_mach_from_A_Astar(A_Astar: float, gamma: float, flowType: FS = FS.SUPER_SONIC) -> float:
