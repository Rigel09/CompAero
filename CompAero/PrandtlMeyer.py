...
class PrandtlMeyer():
    def __init__(self, gamma: float, mach: float = nan, nu: float = nan, mu: float = nan, dwnstreamNu: float = nan, \
                dwnStreamMu: float = nan, dwnStreamMach: float = nan, deflectionAngle: float = nan, inDegrees: bool = True) -> None:
        self.__gamma           = gamma
        self.__mach            = mach
        self.__nu              = nu
        self.__mu              = mu
        ...
        if not self.__degrees: self.__deflectionAngle = degrees(self.__deflectionAngle)

        if not checkValue(self.__gamma): raise ValueError("Valid gamma is required for Prandtl Meyer calculations. Given {}".format(self.__gamma))
        ...
        if checkValue(self.__mach):
            ...
            raise ValueError(Fore.BLACK + Back.RED + "Incorrect Combination of input arguments given to {}".format(self.__class__.__name__) + Style.RESET_ALL + "\t")
    ...
    def __str__(self) -> str:
        ...
        dwnDeflection   = "|{:<{width}}{}|\n".format("Flow Deflection Angle [" + lcg.theta + Misc.degreeSym + "]" , deflectionStr, width=width - len(deflectionStr))

        finish    = "|{:=^{width}}|\n".format("", width=width)

        return "".join([header, sep, gammaS, nuS, muS, sep, jumpSep, dwnMachS, dwnNu, dwnMu, dwnDeflection, finish])
    ...
    def __calculateState(self) -> None:
        ...
        if not checkValue(self.__deflectionAngle): return
